...
S2W_SERIAL_PORT_HANDLE_T  s2wSerialPortHandle;
S2W_SERIAL_PORT_HANDLE_T  s2wSerialPortHandle1;

__no_init S2W_SCAN_DISPLAY_INFO_T scanDisplayInfo[20];
...
#if defined (S2W_I2C_SUPPORT) || defined (S2W_ADC_SUPPORT) || defined (S2W_PWM_SUPPORT)
	CLKCTL_ACM3_BUS_FORCE_SET(CLKCTL);
#endif

#if defined (S2W_SDIO_ENABLE) || defined (S2W_SDIO_ENABLE_1)
...
#if defined (S2W_FS_SPI_ENABLE ) || defined (S2W_FS_SPI_ENABLE_1)
    CLKCTL_AGSPI_BUSCLK_EN_SET (CLKCTL);
#endif
#if defined (S2W_FS_SPI_HI_ENABLE) || defined (S2W_FS_SPI_HI_ENABLE_1)
...
App_MsgProcess(    APP_MAIN_CTX_T *pAppCtx, UINT32 msgId )
{
	...
	UINT8 status;
	UINT8 macAddr[S2W_MAC_ADDR_SIZE] = {0};
	...
	/* associated ch will be at 0 index                */
            ...
            else
            {
			    ...
			    gNwConnConfig.wlanCfg.ssid.length );
			  	...
			  	memcpy(current_wlan_info.bssid, gNwConnConfig.wlanCfg.bssid.array,GSN_WLAN_MAC_ADDR_LEN);
			  	current_wlan_info.validBssid = 1;
			  	if((S2W_WLANDATA_MODE_PROV_AP == s2wCurrent.mode) || (S2W_WLANDATA_MODE_AP == s2wCurrent.mode ))					
    ...
    case APP_EVENT_DHCPv4_IP_CONFLICT:
      ...
      break;
		...
		case APP_NOTIFICATION_WPS_START:
		{
			...
			status = AppS2wHal_WpsAssociate(&pAppCtx->wpsParams,&pAppCtx->wpsapSsid,1);
			if(status != S2W_SUCCESS)
...
#if 0
            AppS2w_WpsAssociateNotify(pAppCtx);
...
#ifdef S2W_15DOT4_SUPPORT        
	else if(S2W_WLANDATA_MODE_ZIP == pIfInfo->macIfType)
	{
...
App_InterfaceStart(APP_MAIN_CTX_T *pAppCtx, APP_IF_INFO_T *pIfInfo)
{
    ...
    if( GSN_WIF_WLAN_NW_TYPE_15_4 == s2wCurrent.mode ||
        S2W_WLANDATA_MODE_ZIP == s2wCurrent.mode )
    {
#if 1      
        GSN_DOT154IF_START_DATA_T *pData = (GSN_DOT154IF_START_DATA_T *)ifcStartReq.reserved;
        pData->NumOfKeyTableEntries = S2W_MAX_NUM_OF_802_15_4_KEY_SUPPORTED;
        pData->NumOfDeviceTableEntries = S2W_MAX_NUM_OF_802_15_4_DEV_SUPPORTED;
        pData->NumOfSecurityLevelEntries = S2W_MAX_NUM_OF_802_15_4_SECURITY_LEVEL_SUPPORTED ;

        pData->modeOfOp = ( S2W_WLANDATA_MODE_ZIP == s2wCurrent.mode ) ? 
...
#ifdef S2W_VARIABLE_PING_PKT_SIZE    
...
s2w_StartDhcpDnsServer()
{
                ...
                if(s2wCurrent.mode == S2W_WLANDATA_MODE_PROV_AP)
                {
                    if(S2W_DEFAULT_GRP_PROV_DHCPSRVR_ENABLE)
                    {
                        memset(&dhcpSrvrCfg,0,sizeof(GSN_DHCP_SRVR_CFG_T));
                        dhcpSrvrCfg.numOfClients = S2W_DEFAULT_GRP_PROV_SUPPORTED_STATIONS;
                        dhcpSrvrCfg.startIpAddr =  S2W_DEFAULT_GRP_PROV_IP_START_ADDRESS;
                        dhcpSrvrCfg.leaseTime = S2W_DEFAULT_GRP_PROV_LEASE_TIME;
                        dhcpSrvrCfg.serverIpAddr = S2W_DEFAULT_GRP_PROV_IP_ADDRESS;
                        dhcpSrvrCfg.startIpAddr = S2W_DEFAULT_GRP_PROV_IP_START_ADDRESS;
                        dhcpSrvrCfg.subnetMask = S2W_DEFAULT_GRP_PROV_SUBNET_ADDRESS;
                        ...
                        if(S2W_DEFAULT_GRP_PROV_DNSSRVR_ENABLE)
                        {
                            ...
                            GsnDnsSrvr_Start(&dnsServer,S2W_DEFAULT_GRP_PROV_DNS_NAME,&gsnDnsInitParams);
                        }
#endif
