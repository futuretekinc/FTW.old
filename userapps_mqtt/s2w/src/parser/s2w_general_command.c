...
#endif
    {"+MQTT_PUBLISH", App2wCmd_mqtt_publish},
#ifdef S2W_DNS_CLIENT
...
UINT8
AppS2wCmd_Wassoc_test(UINT8 *ptr)
{
    S2W_WLANDATA_T params;
#ifndef S2W_IP2WIFI_SUPPORT
    GSN_NWIF_DHCP_PARAMS_T dhcpParams;
...
/*UINT8 MQTT_AP_CONNECT()
{
	...
	SSID = malloc(strlen(AP_SSID));
	WWPA = malloc(strlen(AP_WWPA));
	
	sprintf(WWPA,"%s",AP_WWPA);
...
{	*/
	...
	UINT32 status;
	UINT8 cid;
	temp = malloc(strlen(HOST_ADDRESS)+strlen(HOST_PORT)+1);
	sprintf(temp,"%s,%s",HOST_ADDRESS,HOST_PORT);
    ...
    status = AppS2wHal_NetTcpClient_test(&peerData, &cid);
    if (status != 0)
    {
       	goto error;
    }
...
error:
    ...
    S2W_NETDATA_T peerData;
 	...
 	peerData.ipAddr[0] = 54;
  	peerData.ipAddr[1] = 178;
  	peerData.ipAddr[2] = 154;
  	peerData.ipAddr[3] = 197;
  	peerData.port = 8883;  
  	peerData.proto = S2W_NETDATA_PROTO_TCP;
  	peerData.mode = S2W_NETDATA_MODE_CLIENT;
  	status = AppS2wHal_NetTcpClient_test(&peerData, CID);
...
void TEST_START()
{
  ...
  MQTT_AP_CONNECT();
  GsnTaskSleep(5000);
  MQTT_TCP_CONNECT(&cid);
  GsnTaskSleep(5000);
  MQTT_SSL_CONNECT(cid);
  GsnTaskSleep(5000);
}*/


UINT8
App2wCmd_mqtt_publish(UINT8* ptr)
{
  TEST_START();
  return 0;
}
