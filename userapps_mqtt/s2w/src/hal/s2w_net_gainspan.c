...
PUBLIC UINT8
AppS2wHal_NetTcpClient_test(S2W_NETDATA_T *peerData, UINT8 *cidNo)
{
    struct sockaddr_in destAddr;
    UINT32 cid;
    INT32 sd;
 ...
 ******************************************************************/
extern GSN_OSAL_QUEUE_T MQTT_RECEIVE_QUEUE;
VOID
                        ...
                        if((s2wCidList[cid].sslflag == TRUE) && (s2wCidList[cid].conType == TCP))
                        {
                        	...
                        	buffOfset=0;
                       		while(totalBytesToSnd)
                       		{
                        		...
                        		AppS2wProcess_NetRx(cid, (rxDataBuf+buffOfset), sentLen, 
														(UINT8*)ip, port);
  								buffOfset += sentLen;
...
#ifdef S2W_IPv6_SUPPORT
                          if(s2wCidList[cid].addrType & ADDR_TYPE_IPv6)
                            AppS2wProcess_NetRx(cid, s2wRxBuf, dataLen, (UINT8*)ipv6, port);
                          else
