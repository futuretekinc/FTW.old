...
#include "netx/nx_packet.h"

/* Define to include gain span specific changes.  
*/
#define INCLUDE_GAINSPAN_DEFINED
...
#define NX_BSD_TCP_WINDOW                   (1024*8)                   /* 64k is typical window size for 100Mb ethernet.                       */
#endif 
...
#define NX_BSD_MAX_LISTEN_BACKLOG           32                       /* Maximum listen backlog.                                              */ 
#endif

#ifndef NX_CPU_TICKS_PER_SECOND
#define NX_CPU_TICKS_PER_SECOND             10                     /* Number of timer ticks per second, 100ms timer.                */  
#endif

#ifndef NX_MICROSECOND_PER_CPU_TICK
#define NX_MICROSECOND_PER_CPU_TICK         100000                   /* Number of microseconds per timer interrupt, 100 ms.            */ 
#endif
...
#define SO_UDP_RCVQDEPTH    24  /* Enable setting trasnmit buffer size overriding user limit (admin privelege) */
#define SO_MAX              SO_UDP_RCVQDEPTH/* Maximum Socket option ID      */
...
#ifdef INCLUDE_GAINSPAN_DEFINED/* Gainspan Added Definitions*/

/* NetX event type */
typedef enum NX_BSD_CB_EVENT_TYPE
{
	NX_BSD_CB_TCP_RECIEVE =		(1 << 0),
	NX_BSD_CB_UDP_RECIEVE = 	(1 << 2), 
	NX_BSD_CB_TCP_DISCONNECT = 	(1 << 3),
	NX_BSD_CB_TCP_LISTEN =		(1 << 4),
	NX_BSD_CB_SOCKET_ERROR = 	(1 << 5),
	NX_BSD_CB_TCP_SEND_READY =	(1 << 6),
}NX_BSD_CB_EVENT_TYPE_T;

typedef void (*USER_CALLBACK)(INT socket_id, NX_BSD_CB_EVENT_TYPE_T event, UINT port);

typedef void (*USER_CALLBACK_PARAM)(INT socket_id, NX_BSD_CB_EVENT_TYPE_T event, void *user_ptr);

typedef void (*USER_CALLBACK_TWO_PARAM)(INT socket_id, NX_BSD_CB_EVENT_TYPE_T event, 
											void *user_ptr1, void *user_ptr2);

typedef struct NX_BSD_USER_CB_INFO_STRUCT
{
	INT						cbFlags;/*NX_BSD_CB_EVENT_TYPE_T*/
	VOID *					sockUserPtr1;	/* Call Back ctx */
	VOID *					sockUserPtr2;	/* Call Back ctx */
	USER_CALLBACK			sockUserCBFuncPtr;/*Call back without any CB ctx pointer*/
	USER_CALLBACK_PARAM		sockUserCBParam1FuncPtr;	/*Call back with one CB ctx pointer*/
	USER_CALLBACK_TWO_PARAM	sockUserCBParam2FuncPtr;	/*Call back with two CB ctx pointer*/
        ULONG   connectTimeout;
}NX_BSD_USER_CB_INFO;

NX_TCP_SOCKET * nx_bsd_socket_ptr_get(INT sockID);

NX_PACKET * nx_bsd_packet_ptr_get(INT sockID);
...
#ifdef INCLUDE_GAINSPAN_DEFINED/* Gainspan Added Definitions*/
	NX_BSD_USER_CB_INFO		nx_bsd_cb_info;
#endif	
} NX_BSD_SOCKET;
...
INT nx_bsd_callback_register(INT socket_id, USER_CALLBACK user_callback, INT flags);
INT nx_bsd_callback_register_param(INT socket_id, void *cb_param, USER_CALLBACK_PARAM user_callback, INT flags);
INT nx_bsd_callback_register_param2(INT socket_id, void *cb_param1, void *cb_param2, USER_CALLBACK_TWO_PARAM  user_callback, INT flags);
